...
const TermsAndConditions = ({
  ...
  useEffect(() => {
    ...
    fetch(globalOptions.url + options.path).then(response => response.json()).then(data => setHtmlContent(data[0]['body'])).catch(err => setHtmlContent("<h1>Error Loading Terms and Conditions</h1>"));
  });
  return <View style={_styles.JuDsvOCs}>
      ...
      <ScrollView style={_styles.TuGPvZFb}>
        <HTML source={{
        html: htmlContent
